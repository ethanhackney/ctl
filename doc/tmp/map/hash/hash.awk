...
BEGIN {
  ...
  if (!FUNC) {
    print "hash.awk: no function given"
    exit 1
  }
  if (!UDOC) {
    print "hash.awk: no user doc file given"
    exit 1
  }
  if (!IDOC) {
    print "hash.awk: no internal doc file given"
    exit 1
  }
...
END {
  if (!found) {
    print "hash.awk: could not find \"#define " FUNC "\""
    exit 1
  }
}
